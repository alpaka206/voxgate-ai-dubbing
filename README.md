...
- 더빙 오디오는 전사 word timestamp를 기준으로 segment 단위로 다시 합성해, 문장 사이 pause와 원본 전체 길이를 최대한 유지하도록 처리합니다. 번역 구간이 지나치게 짧을 때는 과한 slowdown 대신 자연스러운 발화 속도를 우선하고, 최종 믹싱에서는 segment 음량이 과하게 작아지지 않도록 `amix` 정규화를 끈 뒤 limiter로 피크를 제어합니다.
...
- 모델이 JSON 뒤에 설명 문장을 덧붙이더라도 첫 유효 JSON 객체만 추려서 해석하도록 번역 응답 파서를 보강했습니다.
